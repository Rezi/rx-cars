...
const buffered = carStream.pipe(buffer(intervalStream));` */
	...
	let freeText = `Delays the emission of items from the source Observable by a given timeout or until a given Date.
	
In this example, values emited in the stream are delayed by ${ANIMATION_DURATION} ms.`;
	...
	function setStreams() {
		...
		const lastCarDelay = (carsStreamDefinition.at(-1)?.delay || 0) + ANIMATION_DURATION * 2;
